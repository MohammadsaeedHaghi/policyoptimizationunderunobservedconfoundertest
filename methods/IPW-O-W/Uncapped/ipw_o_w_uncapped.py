"""R-OW (UNCAPPED) — robust, Odds-box ∩ Wasserstein value maximiser, WITHOUT a capacity constraint.

Identical to the capped R-OW (``methods/IPW-O-W/Capped/ipw_o_w_capped.py``) except that the per-arm
capacity constraint  (1/n) Σ_i π_k(X_i) ≤ cap_k  is **removed**: here the policy may assign any
arm to any fraction of the population. Every other part — the worst-case objective over the
Marginal-Sensitivity box ∩ Wasserstein balls, the simplex, the tie (discretisation) constraints,
and the dual feasibility/transport constraints — is unchanged. The in-sample problem is written
out in ``methods/IPW-O-W/IPW-O-W.html``.
...
# parents: [0]=Uncapped  [1]=R-OW  [2]=methods  [3]=code 1.1
...
@dataclass
class ROWResult:
    """Optimal R-OW policy + dual certificate (no capacity here, so no cap field)."""
...
def solve_ipw_o_w_uncapped(
    ...
    *,
    ...
    See the capped file / ``R-OW.html`` for the full description of ``discretize``, ``mesh``,
