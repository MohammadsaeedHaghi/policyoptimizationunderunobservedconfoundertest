...
if MAIN:
    ...
    if HESSD:
        ...
        try:
            _HCAP = json.load(open(ROOT / "assets/grand/hess_capped.json"))["kz"]
...
projection: two units sharing an index but differing in X<sub>5</sub> necessarily get the same
decision. <br><br><b>Kallus-Zhou do NOT do this.</b> Their code
(<span class="mono">logistic_pol_asgn</span>) optimises &pi;(1|x) = &sigma;(&theta;'x) with
&theta; &isin; R<sup>6</sup> &mdash; five covariates plus an intercept &mdash; over the FULL
vector. The two restrictions are different rather than nested: they can reach any halfspace
DIRECTION in 5-D but only linear policies; we are fully flexible (Lipschitz, any shape) along ONE
direction fixed in advance and blind to the rest. We lose where the informative direction is not
