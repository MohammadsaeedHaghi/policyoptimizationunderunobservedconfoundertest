"""The X-X (unconfoundedness-assuming) methods on any 1-D benchmark, at its report protocol.
...
def shap(Xn,sX,spv,block=200):
    ...
    return out
import argparse
_ap=argparse.ArgumentParser(); _ap.add_argument("--dgp",required=True); _ap.add_argument("--n",type=int,default=400)
_ap.add_argument("--seeds",type=int,default=5); _ap.add_argument("--out",required=True); _a=_ap.parse_args()
sp=importlib.util.spec_from_file_location("d",ROOT+"/"+_a.dgp)
d=importlib.util.module_from_spec(sp); sys.modules["d"]=d; sp.loader.exec_module(d)
N,NTE,SEEDS=_a.n,4000,_a.seeds
...
json.dump(out,open(ROOT+"/"+_a.out,"w"))
print("saved "+_a.out)
