"""Run all 8 methods on the exp_owwin DGP, capped (treat<=50%), N_train=bigN, 8 seeds, Γ∈{1,2,3,5,8,12}.
...
K = d.K; CAP = d.CAP; LV = d.LEVELS; import sys as _sys
NTR = int(_sys.argv[1]) if len(_sys.argv) > 1 else 1500
SEEDS = list(range(5)); GAMMAS = [1.0, 2.0, 3.0, 5.0, 8.0]
...
(HERE / ("owwin_results_N%d.json" % NTR)).write_text(json.dumps(out, indent=2))
print("\nsaved owwin_results_N%d.json" % NTR + "  oracle=%.3f" % orc, flush=True)
