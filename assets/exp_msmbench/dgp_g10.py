"""KMZ'19 DGP at confounding strength log(Gamma*) = 1.0 (Gamma* = e^1.0), per the paper's own
...
_sp = _ilu.spec_from_file_location("kmz_base_g10", str(_P(__file__).resolve().parent / "dgp.py"))
_m = _ilu.module_from_spec(_sp); _sys.modules[_sp.name] = _m; _sp.loader.exec_module(_m)
_m.GSTAR = float(_np.exp(1.0))
