#!/usr/bin/env python3
"""Merge the Gamma=50 column into kmz_main_ce1.0.json (backup kept as *_pre50.json).

The Gamma=50 run writes its own full-shape file; this folds its surface/best columns into the
...
G15 = HERE / "kmz_main_g50only_ce1.0.json"
...
if "50" in m["gammas"]:
    print("Gamma=50 already merged; nothing to do")
    raise SystemExit

shutil.copy(MAIN, HERE / "kmz_main_ce1.0_pre50.json")
gk = [k for k in g["surface"]["IPW-O-W"].keys()]
assert gk == ["50"], gk
for meth in m["methods"]:
    m["surface"][meth]["50"] = g["surface"][meth]["50"]
    b_new = max((g["surface"][meth]["50"][L], L) for L in g["Lgrid"])
    if b_new[0] > m["best"][meth]["value"]:
        m["best"][meth] = {"gamma": "50", "L": b_new[1], "value": round(b_new[0], 4)}
m["gammas"] = m["gammas"] + ["50"]
...
print("merged Gamma=50 into", MAIN.name, "| gammas now", m["gammas"])
