...
<p class="muted"><b>Kallus's row is the reference policy, not the estimator.</b> Their
method minimises worst-case regret against a reference, whose own regret is zero by construction
&mdash; so under real confounding the certified-safe optimum <i>is</i> the reference, and their
synthetic driver anchors it to <span class="mono">ctrl_p_1</span> (never-treat), the worst constant
on this benchmark. Re-running their code with the other references their repository ships:
reference = control &rarr; <b>&minus;1.010</b> (never-treat, exactly) at every
&Gamma;&nbsp;&ge;&nbsp;2; reference = treat-all (their <span class="mono">tmnt_p_1</span>) &rarr;
<b>+1.035</b> (all-treat, exactly); reference = the logging policy &rarr; &asymp;&nbsp;+0.35. A
2.05-point swing from the anchor alone, with the estimator unchanged. The table above reports
their driver's protocol; this note is why that number should not be read as estimator quality.
At &Gamma;&nbsp;=&nbsp;1 the floor stops binding and the method drifts with the noisy point
estimate instead (&minus;0.42 / &minus;0.04 / &minus;0.16).</p>
