"""KMZ'19 DGP at confounding strength log(Gamma*) = 0.5 (Gamma* = e^0.5), per the paper's own
...
_sp = _ilu.spec_from_file_location("kmz_base_g05", str(_P(__file__).resolve().parent / "dgp.py"))
_m = _ilu.module_from_spec(_sp); _sys.modules[_sp.name] = _m; _sp.loader.exec_module(_m)
_m.GSTAR = float(_np.exp(0.5))
