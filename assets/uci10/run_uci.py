#!/usr/bin/env python3
"""Run every method on one UCI semi-synthetic benchmark, over the full Gamma x L x c_eps grid.
...
def draw(seed):
    """Redraw T, both potential outcomes, and the train/test split for this seed."""
    ...
    return dict(Xtr=x[tr].reshape(-1, 1), Ttr=T[tr], Ytr=Yo[tr],
                Xte=x[te], Y0te=Y0[te], Y1te=Y1[te], cate_te=(m1 - m0)[te])
...
def cell(job):
    ...
    if ceps == CEPS[0]:
        ...
        for m in XX:
            try:
                if m == "IPW-X-X": r = S[m](X, T, Y, w, n_arms=K, discretize=False)
                elif m == "DoublyRobust-X-X": r = S[m](X, T, Y, w, mu, n_arms=K, discretize=False)
                else: r = S[m](X, T, Y, n_arms=K, discretize=False)
                xx[m] = {"value": val(r.pi[1]), "curve": [round(float(v), 4) for v in dep(PGRID, r.pi[1])],
                         "support_pi": [round(float(v), 4) for v in r.pi[1]]}
            except Exception as ex:
                print("FAIL %s seed=%d: %s" % (m, seed, str(ex)[:90]), flush=True)
        out["xx"] = xx
...
def main():
    ...
    npz = str(here / "data" / (a.data + "_pop.npz"))
    meta = json.loads((here / "data" / (a.data + ".json")).read_text())

    jobs = [(s, c) for s in range(a.seeds) for c in CEPS]
    t0 = time.time()
    res = {}
    with mp.Pool(min(a.workers, len(jobs)), initializer=_init, initargs=(npz,)) as p:
        for seed, ceps, o in p.imap_unordered(cell, jobs):
    ...
    payload = {"dataset": a.data, "meta": meta, "gammas": GAMMAS,
               ...
               "ceps": CEPS, "n_train": N_TRAIN, "n_test": N_TEST, "seeds": a.seeds,
