"""Add the NON-ROBUST doubly-robust (AIPW) BASELINE to the 1-D 2-arm experiment — the analog of IPW that adds
the estimated outcome model μ̂ but does NOT robustify against the confounder (Γ=1, box collapses to ŵ). Like IPW
it is Γ-independent → a flat baseline line. Computed as the capped DoublyRobust (=R-O-DR) LP at Γ=1. REUSES the
stored NPZ for every other method (incl. the robust DR family). Same seed-0 data / cap / deploy as exp_1d.py.
Usage: python3 exp_1d_add_aipw.py {bern|uni}"""
...
rodr=load("rodr","methods/DoublyRobust-O-X/Capped/doublyrobust_o_x_capped.py").solve_doublyrobust_o_x_capped  # =DoublyRobust; Γ=1 ⇒ plain AIPW
...
d=dict(np.load(ASSETS/NPZ)); GAMMAS=d["GAMMAS"]; mi=4; mG=float(d["matched_Gamma"]); nG=len(GAMMAS)
assert np.allclose(xs,d["xs"])
gi_te=np.array([int(np.argmin(np.abs(xs-x))) for x in te.X.ravel()]); tg=d["RW_treat_grid"]
assert abs(float(((1-tg)[gi_te]*te.Ypot[:,0]+tg[gi_te]*te.Ypot[:,1]).mean())-d["RW_rt"][mi])<1e-9, "scoring gate FAILED"
# ---- AIPW baseline = capped DoublyRobust at Γ=1 (box collapses ⇒ no confounding robustness) ----
sv=rodr(tr.X,tr.T,tr.Y,w,muhat,n_arms=K,Gamma=1.0,cap=CAP,discretize=False)
pi=sv.pi; rt=rt_test(pi); rttr=rt_train(pi); obj=float(sv.objective_value); tgrid=polx(pi)[1]
d["AIPW_rt"]=np.full(nG,rt); d["AIPW_rt_train"]=np.full(nG,rttr); d["AIPW_obj"]=np.full(nG,obj); d["AIPW_treat_grid"]=np.array(tgrid)
np.savez(ASSETS/NPZ,**d)
print(f"[{MODE}][{time.time()-t0:.0f}s] AIPW (DR baseline, Γ=1): rt_train={rttr:.4f} rt_test={rt:.4f} obj={obj:+.4f} "
      f"treat_rate={float(np.mean(tgrid>0.5)):.3f}  vs IPW rt_test={d['IPW_rt'][mi]:.4f}",flush=True)
# ---- regenerate the 4 figures (9 methods; AIPW next to IPW) ----
...
G=GAMMAS
fig,(axL,axR)=plt.subplots(1,2,figsize=(12.8,5.4),dpi=140,sharey=True)
...
h,l=axL.get_legend_handles_labels(); fig.legend(h,l,loc="lower center",ncol=6,fontsize=8,frameon=False,bbox_to_anchor=(0.5,-0.05))
fig.suptitle(f"1-D 2-arm · {STITLE} · realised outcome vs Γ — train vs test  ·  +DoublyRobust +AIPW baseline",y=0.99,fontsize=11.5)
fig.tight_layout(rect=[0,0.09,1,1]); fig.savefig(ASSETS/"realized_train_test.png",bbox_inches="tight"); plt.close(fig)
...
ax.set_title(f"{STITLE} · treatment policy π(treat | x) — all methods at matched Γ={mG}"); ax.grid(alpha=.25); ax.legend(fontsize=8,ncol=2,loc="upper left")
...
(ASSETS/"aipw_chart.json").write_text(json.dumps({"mode":MODE,"x":[round(float(v),4) for v in xs],"gamma":mG,
    "series":{"id":"AIPW","label":"AIPW","color":COL["AIPW"],"y":[round(float(v),4) for v in d["AIPW_treat_grid"]]}}))
(ASSETS/"aipw_summary.json").write_text(json.dumps({"mode":MODE,"AIPW":{"rt_train":round(rttr,4),"rt_test":round(rt,4),"obj":round(obj,4),
    "treat_rate":round(float(np.mean(tgrid>0.5)),3)},"IPW_rt":round(float(d["IPW_rt"][mi]),4)},indent=1))
print(f"[{MODE}][{time.time()-t0:.0f}s] DONE -> AIPW baseline added; npz+csv+json updated, 4 figures regenerated (9 methods)")
