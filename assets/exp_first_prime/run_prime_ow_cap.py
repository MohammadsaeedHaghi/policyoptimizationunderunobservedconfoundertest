"""CAPPED Wasserstein (OW) methods on the first-prime DGP, N_train=1500 (same seed/train/test as run_prime.py).
IPW-O-W and DoublyRobust-O-W, EXACT per-unit transport (discretize=False, NO cell aggregation), treat<=50%.
Solves over the same Γ grid as the XX/OX run (Γ=1 = the capped X-X parent, not re-solved).  ε computed once.
Patches PRIME_RES.cap_value + PRIME_RES.cap_policy in index.html to add the two OW series (blue/green circles)."""
...
K = dgp.K; CAP = dgp.CAP_CAP; NTR, NTE = 1500, 20000
...
anchor = {"IPW-O-W": R0["regimes"]["cap"]["value"]["IPW-X-X"][0],
          "DoublyRobust-O-W": R0["regimes"]["cap"]["value"]["DoublyRobust-X-X"][0]}
pol_anchor = {"IPW-O-W": R0["regimes"]["cap"]["policy"]["IPW-X-X"]["_flat"],
              "DoublyRobust-O-W": R0["regimes"]["cap"]["policy"]["DoublyRobust-X-X"]["_flat"]}
...
for g in GAMMAS:
    ...
    t0 = time.time()
    ro = ipwow(tr.X, tr.T, tr.Y, w, n_arms=K, Gamma=float(g), cap=CAP, discretize=False, zscore=False, epsilon=eps)
    rd = drow(tr.X, tr.T, tr.Y, w, muhat, n_arms=K, Gamma=float(g), cap=CAP, discretize=False, zscore=False, epsilon=eps)
...
(HERE / "prime_ow_cap_results.json").write_text(json.dumps(out, indent=2))
print("\nsaved prime_ow_cap_results.json", flush=True)
...
cv = P["cap_value"]["series"]; cv[:] = [s for s in cv if s["id"] not in COL]
...
for k, sl in P["cap_policy"]["seriesByGamma"].items():
...
P["cap_value"]["note"] = P["cap_value"]["note"] + " Wasserstein (-O-W, circles) added."
