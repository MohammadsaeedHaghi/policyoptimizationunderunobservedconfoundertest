...
and splice them into the first-prime tab (both regimes).  Reads prime_n10000_results.json; run AFTER run_prime.py."""
...
R = json.loads((HERE / "prime_n10000_results.json").read_text())
...
(HERE / "prime_n10000_charts.json").write_text(json.dumps(CH))
...
def table(rname):
    ...
    return ("<table class='restab'>%s%s%s</table>\n<p class='muted' style='margin:6px 0 0'>%s: realised test E[Y] by Γ, N_train=%d (XX+OX only), single seed. Bold = best method at that Γ. XX methods are Γ-free; OX sweep Γ.</p>"
            % (head, "".join(rows), orow, cap, R["N_train"]))
(HERE / "prime_n10000_table_uncap.html").write_text(table("uncap"))
(HERE / "prime_n10000_table_cap.html").write_text(table("cap"))
...
line = "var PRIME_N10K = %s;" % json.dumps(CH, separators=(",", ":"))
if re.search(r'^var PRIME_N10K = .*;$', h, flags=re.M):
    h = re.sub(r'^var PRIME_N10K = .*;$', lambda m: line, h, count=1, flags=re.M)
else:
    h = re.sub(r'^(var PRIME_RES = .*;)$', lambda m: m.group(1) + "\n" + line, h, count=1, flags=re.M)
calls = "\n".join("renderChart('ichart-p10k-%s',PRIME_N10K.%s);" % (k.replace("_", "-"), k) for k in CH)
h = re.sub(r"\nrenderChart\('ichart-p10k-(uncap|cap)-[^\n]*", "", h)
if "renderChart('ichart-p10k-uncap-value'" not in h:
    anchor = "renderChart('ichart-prime-cap-policy',PRIME_RES.cap_policy);"
