...
def truth(p):
    eY0 = np.zeros_like(LV); eY1 = p["b"] * (p["c"] - LV**2)   # E[S|X]=0 -> S terms vanish in the X-marginal
    return eY0, eY1
def job(args):
    ...
    for cs in (1.2, 1.8):
        for ce in (2.0, 3.0):
            for d0 in (2.0, 3.0):
                for dd in (0.5, 1.0):
                    for b in (2.0, 3.0):
                        CONFIGS["cs%g_ce%g_d%g_dd%g_b%g" % (cs, ce, d0, dd, b)] = dict(
                            cs=cs, ce=ce, d0=d0, d1=d0 + dd, b=b, c=0.3, noise=0.6)
    names = list(CONFIGS)
