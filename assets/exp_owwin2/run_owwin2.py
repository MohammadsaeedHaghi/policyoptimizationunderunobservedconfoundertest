...
Exact analytic value. Saves owwin2_results.json: per-regime per-method 5-seed mean/sd by Γ + seed-0 policies + oracle."""
...
K = d.K; LV = d.LEVELS; NTR = 700; SEEDS = list(range(5)); GAMMAS = [1.0, 2.0, 4.0, 8.0, 20.0, 100.0, 1000.0]
...
for reg in ["uncap", "cap"]:
    ...
    out["regimes"][reg] = {"mean": {m: [round(float(v), 4) for v in np.array(per[m]).mean(0)] for m in ALL},
                           ...
                           "policy_by_seed": polS}
(HERE / "owwin2_results.json").write_text(json.dumps(out, indent=2))
print("\nsaved owwin2_results.json  oracle=%.3f" % orc, flush=True)
for reg in ["uncap", "cap"]:
