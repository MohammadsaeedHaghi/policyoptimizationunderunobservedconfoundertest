"""Build the exp_owwin2 (E3) tab: data-explanation charts + per-regime value-vs-Γ (x=log10 Γ, to 1000, shows the
collapse) + π policy + mean±SD tables. Splice OWWIN2_CHARTS + render calls + tables into index.html."""
...
HERE = ROOT / "assets" / "exp_owwin2"
sp = importlib.util.spec_from_file_location("ow2dgp", str(HERE / "dgp.py")); d = importlib.util.module_from_spec(sp); sys.modules["ow2dgp"] = d; sp.loader.exec_module(d)
R = json.loads((HERE / "owwin2_results.json").read_text())
...
XL = [round(math.log10(g), 4) for g in GAMMAS]
gticks = "Γ: " + ", ".join(str(int(g)) for g in GAMMAS)
def value_chart(reg):
    M = R["regimes"][reg]["mean"]
    return {"x": XL, "xmin": min(XL), "xmax": max(XL), "xlabel": "log₁₀ Γ   (" + gticks + ")", "ylabel": "realised E[Y] (5-seed mean)",
            ...
            "note": "%s: realised E[Y] vs Γ (to 1000; x=log10 Γ), N=%d, 5-seed mean. XX<OX<OW at moderate Γ; as Γ grows the robust methods over-assume and COLLAPSE (value falls toward never-treat)." % ("UNCAPPED" if reg == "uncap" else "CAPPED (treat<=50%)", R["N_train"])}
...
(HERE / "owwin2_charts.json").write_text(json.dumps(W))
...
(HERE / "owwin2_table_uncap.html").write_text(table("uncap"))
(HERE / "owwin2_table_cap.html").write_text(table("cap"))
...
line = "var OWWIN2_CHARTS = %s;" % json.dumps(W, separators=(",", ":"))
if re.search(r'^var OWWIN2_CHARTS = .*;$', h, flags=re.M):
    h = re.sub(r'^var OWWIN2_CHARTS = .*;$', lambda m: line, h, count=1, flags=re.M)
else:
    h = re.sub(r'^(var OWWIN_CHARTS = .*;)$', lambda m: m.group(1) + "\n" + line, h, count=1, flags=re.M)
calls = "\n".join("renderChart('ichart-ow2-%s',OWWIN2_CHARTS.%s);" % (k, k) for k in W)
h = re.sub(r"\nrenderChart\('ichart-ow2-[^\n]*", "", h)
if "renderChart('ichart-ow2-outcome'" not in h:
    anchor = "renderChart('ichart-ow-outcome',OWWIN_CHARTS.outcome);"
    assert anchor in h, "owwin anchor missing"; h = h.replace(anchor, anchor + "\n" + calls, 1)
for ph, fn, bid in [("OW2_TBL_UNCAP", "owwin2_table_uncap.html", "restab-ow2-uncap"), ("OW2_TBL_CAP", "owwin2_table_cap.html", "restab-ow2-cap")]:
...
idx.write_text(h)
print("spliced OWWIN2 (%d charts) + tables" % len(W))
for reg in ("uncap", "cap"):
