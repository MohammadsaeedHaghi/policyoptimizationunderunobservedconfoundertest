#!/usr/bin/env python3
"""DECOMPOSITION ARM: SharpHess with the FIXED optimiser but the campaign's ORIGINAL k=15.
...
def main():
    ...
    t0 = time.time()
    sc = H.fit_scores(X, T, Y, Gamma=G, k=15, n_folds=2, maximize=True, seed=int(d["seed"]))
    ...
    # separate key: this arm exists ONLY to separate the optimiser fix from the k change, and
    # must not disturb the campaign's reported `hess` field.
    old = d.get("hess", {}).get("%g" % G, {}).get("value", float("nan"))
    d["hess_fixedopt_k15"] = {"%g" % G: new}
