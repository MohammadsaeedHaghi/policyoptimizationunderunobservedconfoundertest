#!/usr/bin/env python3
"""Recompute the Kallus arm with the PAPER-EXACT port of the KZ18 authors' code.
...
def main():
    ...
    _sp = _iu.spec_from_file_location("kal_p", "/home1/haghim/code 1.1/methods/Kallus/kallus.py")
    _km = _iu.module_from_spec(_sp); sys.modules["kal_p"] = _km; _sp.loader.exec_module(_km)
    r = _km.fit_kallus_paper(X, T, Y, wraw, n_arms=2, Gamma=G, maximize=True, seed=int(d["seed"]))
    pe = _km.predict_kallus_paper(r.theta, Xte.reshape(-1, 1))[:, 1]
    new = {"value": float(np.mean(pe * Y1t + (1 - pe) * Y0t)),
           "objective": float(r.objective_value),
           "pe_mean": float(pe.mean()), "pe_sd": float(pe.std())}

    if "kallus_customfix" not in d:              # keep the custom-optimiser number, once
        d["kallus_customfix"] = d.get("kallus", {})
    old = d.get("kallus", {}).get("%g" % G, {}).get("value", float("nan"))
    d["kallus"] = {"%g" % G: new}
    d["kallus_optimiser"] = {"recipe": "kz18-paper", "restarts": 15, "armijo": True,
                             "averaging": "polyak+meanloss", "fixed": "2026-08-06"}
