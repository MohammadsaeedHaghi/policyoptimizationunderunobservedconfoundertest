#!/usr/bin/env python3
"""FILL the k-NN-nuisance SharpHess arm on cells that only have the neural one (gamma 3 and 4).
...
def main():
    ...
    t0 = time.time()
    sc = H.fit_scores(X, T, Y, Gamma=G, k=15, n_folds=2, maximize=True,
                      seed=int(d["seed"]), nuisance="knn")
    ...
    old = d.get("hess_knn", {}).get("%g" % G, {}).get("value", float("nan"))
    d["hess_knn"] = {"%g" % G: new}          # fills the comparison arm; `hess` (neural) untouched
    d["hess_knn_optimiser"] = {"nuisance": "knn", "k": 15, "n_iter": 800, "lr": 2.0,
                               "restarts": 6, "normalize_grad": True, "select": "best"}
