...
<div class="card"><b>Provenance note (corrected).</b> This construction family (subsample an
RCT with a rule on a then-hidden variable) follows Frauen et al. 2025 (arXiv 2502.13022), who
hide diastolic blood pressure in IST; we hide baseline consciousness (deliberately MORE
prognostic, S&rarr;Y = +0.34) and calibrate the selection to the matched-&Gamma; protocol
(&Lambda; = 4.95) rather than using fixed drop rates. Kallus &amp; Zhou's own case study is WHI
(restricted access), not IST, so no published Kallus-method IST numbers exist to compare
against.</div>
